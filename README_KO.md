...
- [x] **Android Vita3K 배포 트러블슈팅 및 근본 원인 해결 (2026-08-09).** 사용자가
  Android에서 패치 적용 후 "설치 목록에 안 뜸" → "검은 화면" → "파일 로드
  실패 에러" → "파일 정확히 교체됐는데도 여전히 일본어로 나옴"까지 여러
  단계의 문제를 순차적으로 겪음. 여러 잘못된 가설(zip eboot.bin이 아직
  암호화된 SELF 상태였던 문제, sce_pfs DRM 무결성 검증이 크기 다른
  `toidata_release.l7c`를 설치 시점에 거부하는 문제 등은 실제 원인이었음)을
  거쳐 **최종 근본 원인을 확인**: 순수 원본(패치 도구를 한 번도 안 거친)
  `toidata_release.l7c`를 Vita3K가 부팅 중 열면(`sceIoOpen`) **에러 없이
  조용히 멈추는 버그**가 있음(Windows에서 직접 재현·확인, CPU만 계속 쓰며
  로그도 그 지점에서 멈춤). "파일이 안 바뀐 것처럼 보이는" 증상은 실제로는
  이 행(hang) 때문에 마지막 렌더링된 화면(원문)이 남아있던 것이었음. 해결책:
  `_Data`/`eboot.bin`뿐 아니라 `toidata_release.l7c`도 **반드시 우리 패치
  도구(`l7ca_patch_multi.py`)를 거친 버전**으로 교체해야 함(내용이 얼마나
  바뀌었든 상관없이, 도구를 거치기만 하면 됨) — Windows/Android 양쪽에서
  최종 부팅·한글 출력 확인 완료. 자세한 진단 과정은 `NEXT_SESSION_PROMPT.md`,
  적용 방법은 `PATCH_적용법.md` 참고.
